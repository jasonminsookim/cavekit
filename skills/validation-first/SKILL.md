...
description: |
  ...
  Covers the 6-gate validation pipeline, phase gates between SPIIM phases, merge protocol,
...
## The 6-Gate Validation Pipeline

Every implementation must pass through six gates, in order. Earlier gates are cheaper and faster — catch problems early.

### Gate 1: Build

**What:** The project compiles/builds without errors.
...
| Gate | What | Command Pattern | Speed | Automated? |
|------|------|----------------|-------|------------|
| 1. Build | Compiles without errors | `{BUILD_COMMAND}` | Seconds | Yes |
| 2. Unit Tests | Functions work correctly | `{TEST_COMMAND}` | Seconds-Minutes | Yes |
| 3. E2E/Integration | Components work together | `{E2E_TEST_COMMAND}` | Minutes | Yes |
| 4. Performance | Meets speed/resource thresholds | `{BENCHMARK_COMMAND}` | Minutes | Yes |
| 5. Launch | Application starts and runs | `{START_COMMAND}` + health check | Seconds | Yes |
| 6. Human Review | Meets intent and quality bar | Human inspection | Variable | No |
...
## Phase Gates Between SPIIM Phases

Phase gates are mandatory verification checkpoints between SPIIM phases. They ensure that the output of one phase is solid before the next phase builds on it.
...
Validation gates operate continuously across all SPIIM phases. Phase gates control transitions between phases. The iteration loop uses gate results as convergence signals.
...
2. **6 gates in order:** Build → Unit Tests → E2E/Integration → Performance → Launch → Human Review
...
5. **Phase gates control SPIIM transitions** — do not proceed until the current phase passes its gate
