...
Instead of documentation flowing from code to a wiki for humans -- where it rots -- invert it:
create agent-consumable documentation that teaches AI how to explore codebases and find current
information on demand.

## Core Principle

> **Documentation is increasingly built for agents, not people. Structure for machine consumption
> (hierarchical, cross-referenced, clear entry points) rather than narrative human reading.**
...
```
Code changes → Engineer writes wiki page → Wiki sits on Confluence/Notion
                                            ↓
                                     Months pass
                                            ↓
                                     Code changes again
                                     ...
                                     Nobody updates it
                                            ↓
                                     New engineer reads stale docs
                                            ↓
                                     Confusion, wasted time, wrong assumptions
```
...
```
Code changes → CLAUDE.md per module updated alongside code
                     ↓
              Skill per library teaches agent navigation
                     ↓
              Plugin packages skills for on-demand loading
                     ↓
              Agent encounters library → loads skill → explores CURRENT source
                     ↓
              Documentation is always current because it IS the codebase guidance
```
...
| Aspect | Human-Narrative | Machine-Structured |
|--------|----------------|-------------------|
| Format | Paragraphs, story flow | Tables, lists, labeled sections |
| Entry points | Mentioned in passing | Explicit section with file paths |
| Types | Described in prose | Listed with locations |
| History | Included (context for humans) | Omitted (irrelevant to agents) |
| Conventions | Implied or in a separate style guide | Explicit rules in the CLAUDE.md |
| Cross-references | "See also the auth docs" | `See ../api/CLAUDE.md#authentication` |
...
| SDD Concept | Documentation Inversion Equivalent |
|------------|-----------------------------------|
| Context directory (specs/, plans/, impl/) | CLAUDE.md files per module |
| Progressive disclosure (index → sub-docs) | CLAUDE.md hierarchy (root → module → sub-module) |
| Specs as primary artifact | CLAUDE.md as primary documentation artifact |
| Skills as reusable knowledge | Navigation skills as reusable library guidance |
| Plugins as distribution | Documentation plugins as distributable library docs |

The key insight from SDD applies directly: **if your documentation is good enough, an agent
can navigate and contribute to the codebase without human hand-holding.**
...
- **sdd-methodology** -- How documentation inversion fits into the broader SDD lifecycle
