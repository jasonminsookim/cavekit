...
**Tried:** Implemented RS256 JWT tokens with public/private key pair.
**Failed because:** Key rotation complexity exceeded the session management requirements. Added 200+ lines of key management code for no user-visible benefit. Symmetric HS256 with server-side session store is simpler and meets all spec criteria.
**Do not retry.** Use symmetric tokens with server-side sessions instead.

### DE-2: Redis for session storage
**Tried:** Used Redis as the session store backend.
**Failed because:** Redis dependency adds operational complexity. The application's expected concurrent user count (< 10,000) is well within what a database-backed session store handles. Redis would be premature optimization.
**Do not retry** unless concurrent user requirements change significantly (> 100,000).

### DE-3: Cookie-based CSRF protection
**Tried:** Double-submit cookie pattern for CSRF.
**Failed because:** Incompatible with the API's token-based auth flow. Clients send bearer tokens in headers, not cookies. CSRF protection is inherently handled by the token-based approach.
**Do not retry.** Token-based auth does not need separate CSRF protection.
...
**Tried:** {What was actually attempted — be specific}
**Failed because:** {Root cause of failure — not just "it didn't work"}
**Do not retry.** {What to do instead, or under what conditions retry would make sense}
```
...
| `code-changes` | true, false | Whether code was modified (helps humans prioritize review) |
| `summary` | Free text | What was accomplished — be specific |
| `blockers.type` | NONE, TEMPORARY, PERMANENT | TEMPORARY = will resolve on its own; PERMANENT = requires human action |
| `recommendation` | Free text | What the next session should do with this item |
...
- **Work queue generation:** The `/sdd:next-session` command consumes this feedback to generate prioritized work items
...
- **Effort:** M
- **Impact:** medium
- **Plan reference:** plan-testing.md
...
This eliminates 25-30 minutes of discovery overhead at the start of each session. The next agent reads this file and begins working immediately on the highest-priority item.
...
Implementation tracking is used primarily during the Implement and Iterate phases of SPIIM. The iteration loop reads and updates tracking documents every pass. The Monitor phase reviews tracking documents for progress signals.
