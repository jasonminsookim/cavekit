...
description: >
  ...
  dramatically -- a 3-stage pipeline that takes 10 hours sequentially completes in ~6 hours
  with leader-follower staggering.
  Triggers: "leader follower", "staggered pipeline", "parallel prompts with delay",
...
```
Stage 1: Specs     ████████████████                              (4 hours)
Stage 2: Plans                     ████████████████              (4 hours)
Stage 3: Implement                                ████████████   (2 hours)
                   ─────────────────────────────────────────────
                   Total: 10 hours
```

### Leader-Follower (Staggered)

```
Stage 1: Specs     ████████████████                              (4 hours)
Stage 2: Plans          ████████████████                         (4 hours, started 1h after Stage 1)
Stage 3: Implement           ████████████                        (2 hours, started 2h after Stage 1)
                   ─────────────────────────────────────────────
                   Total: ~6 hours
```
...
1. **Stage 2 starts after 1-hour delay.** By then, Stage 1 has produced partial specs.
2. **Stage 2 builds plans from whatever specs exist.** Some plans will be based on incomplete specs.
3. **Stage 1 continues producing specs.** On Stage 2's next convergence iteration, it reads
   the newly completed specs and corrects its plans.
4. **Stage 3 starts after 2-hour delay.** By then, both specs and plans are partially complete.
5. **All stages converge.** Each iteration reads the latest upstream artifacts. Errors introduced
   by working from partial input are corrected on subsequent iterations.

The key mechanism is **convergence** -- the iterative loop that re-reads inputs each pass. Without
convergence loops, leader-follower would produce garbage. With them, early errors wash out over
iterations.
...
- `-d 1h` -- Delay before starting (leader-follower offset)
...
|------|-----------------|-----------------|---------------------|
| 0:00 | Starts. Reads refs, begins generating specs. | Waiting (1h delay). | Waiting (2h delay). |
| 1:00 | Iteration 1 complete. ~60% of specs written. Committed. | Starts. Reads partial specs, begins generating plans. | Waiting. |
| 2:00 | Iteration 2. Specs ~85% complete. | Iteration 1 complete. Plans based on 60% specs. Some plans will need correction. | Starts. Reads partial specs + plans, begins implementing. |
| 3:00 | Iteration 3. Specs ~95% complete, converging. | Iteration 2. Re-reads updated specs. Corrects plans. Plans ~70% correct. | Iteration 1 complete. Some implementation based on wrong plans. |
| 4:00 | Converged. Specs complete. Done. | Iteration 3. Re-reads final specs. Plans ~90% correct. | Iteration 2. Re-reads corrected plans. Fixes implementation. |
| 5:00 | -- | Iteration 4. Plans converged. Done. | Iteration 3. Implementation ~80% correct. |
| 6:00 | -- | -- | Iteration 4-5. Implementation converges. Done. |

**Result: ~6 hours total instead of ~10 hours sequential.**
...
## When Leader-Follower Works Best
...
## Monitoring Leader-Follower Execution
...
A leader-follower pipeline has converged when:
...
In multi-agent setups, leader-follower applies at the pipeline level, not the agent team level:

```
Pipeline Level (leader-follower timing):
  Stage 1 (Specs)     → Single agent or agent team
  Stage 2 (Plans)     → Single agent or agent team (starts after delay)
  Stage 3 (Implement) → Agent team with worktree isolation (starts after delay)
```

Each stage can internally use agent teams (multiple teammates working in parallel on different
domains), but the *stages themselves* are staggered using leader-follower timing.
...
When setting up a leader-follower pipeline:
...
- **sdd-methodology** -- Where leader-follower fits in the SPIIM lifecycle
