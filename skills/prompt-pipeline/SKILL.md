...
description: >
  How to design the numbered prompt pipeline that drives SPIIM phases in Spec-Driven Development.
  ...
  Trigger phrases: "prompt pipeline", "design prompts for SDD", "create SPIIM prompts",
  "pipeline prompts", "how many prompts do I need"
---

# Prompt Pipeline Design

The prompt pipeline is the engine of SDD. Each numbered prompt drives one phase of the SPIIM lifecycle (Spec, Plan, Implement, Iterate, Monitor). Prompts are structured markdown files that instruct an AI agent to perform a specific phase, with detailed information delegated to specs, plans, and reference materials.
...
| Prompt | SPIIM Phase | Input | Output | What Happens |
|--------|-------------|-------|--------|--------------|
| `001-generate-specs-from-refs.md` | **Spec** | `context/refs/` | `context/specs/` | Reads reference materials, decomposes into domain-specific specs with cross-references and testable acceptance criteria |
| `002-generate-plans-from-specs.md` | **Plan** | `context/specs/` + `context/impl/` | `context/plans/` | Reads specs plus implementation progress, creates framework-specific plans with feature dependencies, test strategies, and acceptance criteria |
| `003-generate-impl-from-plans.md` | **Implement** | `context/plans/` + `context/specs/` | `src/`, `tests/`, `context/impl/` | Implements the highest-priority unblocked work from plans, runs tests, updates implementation tracking |

### Key behaviors

- **Prompt 001** runs once or a few times to stabilize specs. It reads `context/refs/` and produces `context/specs/`.
...
1. Create `context/specs/spec-overview.md` as the index file
2. Create one `context/specs/spec-{domain}.md` per domain
...
- [ ] spec-overview.md indexes all spec files with one-line summaries
...
| `002-generate-specs.md` | Spec | Feature scope + reference materials | `shared-context/specs/` (implementation-agnostic specs) |
...
| Principle | Detail |
|-----------|--------|
| **One prompt per SPIIM phase** | Each prompt maps to exactly one phase. Do not combine phases. |
...
| **Exit criteria with completion signals** | Every prompt ends with a checklist of exit criteria and a completion signal string (e.g., `<all-tasks-complete>`). |
...
```
Prompt 002 (Plans):
  READS:  context/specs/     (what to build)
...
Prompt 003 (Implement):
  ...
  READS:  context/specs/     (acceptance criteria)
...
1. context/specs/spec-{DOMAIN}.md (WHAT to build)
...
- If blocked for more than 15 minutes, document the blocker and stop
```
...
- If a task takes more than 15 minutes, document findings and move on
...
|----------|--------|----------|
| **Mechanical** | 5 minutes | File creation, boilerplate, simple refactors |
| **Investigation** | 15 minutes | Debugging, researching APIs, understanding existing code |
| **Category budget** | 15 minutes | Total time for all tasks in one category before escalating |
...
1. **Set expectations in the prompt:**
   ```markdown
   ## Time Guards
   - Mechanical tasks (file creation, boilerplate): 5 min max
   - Investigation tasks (debugging, research): 15 min max
